###############################################################
# TITANIC END TO END FEATURE ENGINEERING AND DATA PREPROCESSING
###############################################################
...
# age level
df.loc[(df["AGE"] > 0) & (df['AGE'] < 18), 'NEW_AGE_CAT'] = 'young'
...
# df.drop("NEW_Age_PCLASS", axis=1, inplace=True)
...
binary_cols = [col for col in df.columns if df[col].dtype not in ["int32", "int64", "float"]
...
num_cols = [col for col in df.columns if col not in "PASSENGERID"]
...
#############################################
# 7. Standard Scaling (Bu problemde gerekli değil)
#############################################
